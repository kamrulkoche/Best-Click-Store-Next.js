...
const handleLogout = () => {
  ...
  const handleDeleteUser = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/employee/users/${id}`);
...
{jsonData !== null && (
    <div>
      ...
      " align="center" >All Employee  List</h1>

        <h3 align="right">
          <input type="text" value={searchQuery}
            placeholder="Search" className="input input-bordered input-sm w-full max-w-xs"
        ...
        <table className="table">
          <tbody>
            ...
            {filteredData.map((item) => (
              <tr key={item.id} className="bg-white-200">
                <td>
...
export async function getServerSideProps() {
  const response = await axios.get("http://localhost:3000/employee/registrationallview");
